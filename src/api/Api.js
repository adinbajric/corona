...
const urlNews =
  "https://newsapi.org/v2/everything?q=Covid=2021-05-01&sortBy=popularity&language=en&apiKey=";
...
export const fetchNews = async () => {
  try {
    const {
      data: { articles },
    } = await axios.get(`${urlNews}${newsApiKey}`);
...
export const countries = async () => {
  try {
    const {
      data: { countries },
    } = await axios.get(`${urlCovidStatistics}/countries`);
